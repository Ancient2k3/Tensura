...
function _main_init()
  ...
  if plr and plr.Character and target and hmoid and hmoid.Health > 0 then
    if not vars.is_low_health then
...
if hrp then
  ...
  vars.current_void_position = hrp.Position.Y - 95
end

_chat_str("[MODEL_D2: AI Auto-Battle Started!]")
_chat_str("[DEVELOPED BY: Just_wasting_time]")
_chat_str("[ANTI VOID (UPDATED): " .. tostring(vars.current_void_position) .. " ]")
