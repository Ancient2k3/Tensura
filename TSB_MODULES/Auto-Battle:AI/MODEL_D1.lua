...
function _main_init()
  ...
  if hmoid and hmoid.Health < 10 and hmoid.Health > 0 then
  ...
  else if vars.is_low_health then vars.is_low_health = false
      _normal_tpto(vars.oldest_position)
      _chat_str("Healing complete... now back to battlefield!")
    end
...
_chat_str("[MODEL_E7: AI Auto-Battle Started!]")
_chat_str("[DEVELOPED BY: ???]")
